...
PyBass.Fretboard = function (paper, fb_length, fb_width) {
  ...
  for (var fret = 0; fret < total_frets; fret += 1) {
    ...
    if (fret_num == 3) {
      var circ = paper.circle(previous_d + (d - previous_d) / 2, (fb_width / 2), fret_marker_radius).attr({
    ...
    if (fret_num == 12) {
      var circ = paper.circle(previous_d + (d - previous_d) / 2, (fb_width / 4), fret_marker_radius).attr({
        fill: fret_marker_color,
        stroke: 'none',
        opacity: fret_marker_opacity
      });
      var circ = paper.circle(previous_d + (d - previous_d) / 2, (fb_width - (fb_width / 4)), fret_marker_radius).attr({
        fill: fret_marker_color,
        stroke: 'none',
        opacity: fret_marker_opacity
      });
...
c1.mouseover(function () {
    ...
    this.animate({opacity: 50}, 100, function () { this.animate({opacity: 100}, 100) });
});

c1.vmouseover(function () {
    this.attr("fill", "#000000");
});
c1.vmouseout(function () {
